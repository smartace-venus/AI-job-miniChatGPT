...
export default function LawyerDocsPage() {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof CommonDocument; direction: 'ascending' | 'descending' } | null>(null);
  ...
  const handleSort = (key: keyof CommonDocument) => {
            ...
            {isLoading ? (
              ...
              filteredDocs.map((doc) => (
                <TableRow key={doc.id} className="hover:bg-accent/50">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <span>{doc.admin_name || t('Unknown admin')}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-right">
                    <DropdownMenu>
